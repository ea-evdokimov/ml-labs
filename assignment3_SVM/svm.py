...
def rbf(x_1, x_2, sigma=1.):
    ...
    '''
    distances = torch.exp(-torch.cdist(x_1, x_2, p=2) ** 2 / sigma ** 2)
    return distances
...
def hinge_loss(scores, labels):
    ...
    # print(labels.shape)
    # print(scores.shape)
    return torch.mean(torch.max(torch.zeros(scores.shape), 1 - labels * scores))
...
class SVM(BaseEstimator, ClassifierMixin):
    ...
    def predict(self, batch):
        ...
        print(answers.shape, scores.shape)
        answers[scores > 0] = 1
        return answers